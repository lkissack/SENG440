...
filter_init:
	...
	str	r0, [fp, #-8]	@ X, X
	str	r1, [fp, #-12]	@ Y, Y
@ iir_custom.c:30: 	X[0] = X[1] = (short int) 0x8001;
...
@ iir_custom.c:32: 	for(i =2; i<128; i++){
	mov	r4, #2	@ i,
@ iir_custom.c:32: 	for(i =2; i<128; i++){
	b	.L5		@
.L6:
@ iir_custom.c:34: 		X[i] = (short int)0x7FFF;
	mov	r3, r4	@ i.1_3, i
...
@ iir_custom.c:32: 	for(i =2; i<128; i++){
	cmp	r4, #127	@ i,
	ble	.L6		@,
@ iir_custom.c:36: 	Y[0] = Y[1] = (short int)0x8001;
...
print:
	...
	sub	sp, sp, #24	@,,
	str	r0, [fp, #-8]	@ i, i
@ iir_custom.c:51: 	printf("Y[%2d]_=_%+6hi_=_%04hX_......._x[%2d]_=_%8.5f\n",i, Y[i], Y[i],i, ((float)Y[i])/32768);
...
main:
	...
	add	fp, sp, #24	@,,
	sub	sp, sp, #44	@,,
@ iir_custom.c:56: 	for(run = 0; run<10000; run++){
	mov	r3, #0	@ tmp207,
	str	r3, [fp, #-32]	@ tmp207, run
@ iir_custom.c:56: 	for(run = 0; run<10000; run++){
	b	.L15		@
.L38:
@ iir_custom.c:59: 	const short int C0 = 0x10C8;
	movw	r3, #4296	@ tmp209,
	strh	r3, [fp, #-34]	@ movhi	@ tmp208, C0
@ iir_custom.c:60: 	const short int C1 = 0x2190;
	movw	r3, #8592	@ tmp211,
	strh	r3, [fp, #-36]	@ movhi	@ tmp210, C1
@ iir_custom.c:61: 	const short int C2 = 0x10C8;
	movw	r3, #4296	@ tmp213,
	strh	r3, [fp, #-38]	@ movhi	@ tmp212, C2
@ iir_custom.c:62: 	const short int C3 = 0x5FB7;
	movw	r3, #24503	@ tmp215,
	strh	r3, [fp, #-40]	@ movhi	@ tmp214, C3
@ iir_custom.c:63: 	const short int C4 = 0xDD28;//Negative representation
	movw	r3, #56616	@ tmp217,
	movt	r3, 65535	@ tmp217,
	strh	r3, [fp, #-42]	@ movhi	@ tmp216, C4
@ iir_custom.c:69: 	filter_init_better(X.Init, Y);
	movw	r1, #:lower16:Y	@,
	movt	r1, #:upper16:Y	@,
	movw	r0, #:lower16:X	@,
	movt	r0, #:upper16:X	@,
	bl	filter_init_better		@
@ iir_custom.c:71: 	for (i =2; !(i&0x80); i+=2){
	mov	r4, #2	@ i,
@ iir_custom.c:71: 	for (i =2; !(i&0x80); i+=2){
	b	.L16		@
.L37:
@ iir_custom.c:74: 		register int Xi = X.input[i];
...
@ iir_custom.c:74: 		register int Xi = X.input[i];
	mov	r8, r3	@ Xi, _1
@ iir_custom.c:75: 		register int Xi_minus_1 = X.input[i-1];	
	sub	r2, r4, #1	@ _2, i,
@ iir_custom.c:75: 		register int Xi_minus_1 = X.input[i-1];	
...
@ iir_custom.c:76: 		register int t2 = (int)C2 *(int)X.input[i-2] + (1<<14);
...
@ iir_custom.c:77: 		tmp_2 = ROUND_SHIFT(t2);
	ros	r3, r3
	str	r3, [fp, #-48]	@ iftmp.2_81, tmp_2
@ iir_custom.c:79: 		register int Yi_minus_1 = Y[i-1];
	sub	r2, r4, #1	@ _11, i,
@ iir_custom.c:79: 		register int Yi_minus_1 = Y[i-1];
...
@ iir_custom.c:80: 		register int t4 = (int)C4 *(int)Y[i-2] + (1<<14);
...
@ iir_custom.c:81: 		tmp_4 = ROUND_SHIFT(t4);
	ros	r3, r5		
	str	r3, [fp, #-52]	@ iftmp.3_82, tmp_4
@ iir_custom.c:85: 		register int t0 = (int)C0 *(int)Xi + (1<<14);
	ldrsh	r3, [fp, #-34]	@ _20, C0
@ iir_custom.c:85: 		register int t0 = (int)C0 *(int)Xi + (1<<14);
	mul	r3, r3, r8	@ _21, _20, Xi
@ iir_custom.c:85: 		register int t0 = (int)C0 *(int)Xi + (1<<14);
	add	r5, r3, #16384	@ t0, _21,
@ iir_custom.c:86: 		tmp_0 = ROUND_SHIFT(t0);
	ros	r3, r5 		
	str	r3, [fp, #-56]	@ iftmp.4_83, tmp_0
@ iir_custom.c:87: 		register int t1 = (int)C1 *(int)Xi_minus_1 + (1<<14);
	ldrsh	r3, [fp, #-36]	@ _24, C1
@ iir_custom.c:87: 		register int t1 = (int)C1 *(int)Xi_minus_1 + (1<<14);
	mul	r3, r3, r7	@ _25, _24, Xi_minus_1
@ iir_custom.c:87: 		register int t1 = (int)C1 *(int)Xi_minus_1 + (1<<14);
	add	r5, r3, #16384	@ t1, _25,
@ iir_custom.c:88: 		tmp_1 = ROUND_SHIFT(t1);
	ros	r3, r5
	str	r3, [fp, #-60]	@ iftmp.5_84, tmp_1
@ iir_custom.c:89: 		register int t3 = (int)C3 *(int)Yi_minus_1 + (1<<14);
	ldrsh	r3, [fp, #-40]	@ _28, C3
@ iir_custom.c:89: 		register int t3 = (int)C3 *(int)Yi_minus_1 + (1<<14);
	mul	r3, r3, r6	@ _29, _28, Yi_minus_1
@ iir_custom.c:89: 		register int t3 = (int)C3 *(int)Yi_minus_1 + (1<<14);
	add	r5, r3, #16384	@ t3, _29,
@ iir_custom.c:90: 		tmp_3 = ROUND_SHIFT(t3);
	ros	r3, r5
	str	r3, [fp, #-64]	@ iftmp.6_85, tmp_3
@ iir_custom.c:93: 		Y[i] = (short int) SHORT_INT_CLIPPING(tmp_0 +tmp_1 + tmp_2 + tmp_3 + tmp_4);
...
@ iir_custom.c:95: 		t0 = (int)C0 *(int)X.input[i+1] + (1<<14);
	ldrsh	r2, [fp, #-34]	@ _42, C0
@ iir_custom.c:95: 		t0 = (int)C0 *(int)X.input[i+1] + (1<<14);
	add	r1, r4, #1	@ _43, i,
@ iir_custom.c:95: 		t0 = (int)C0 *(int)X.input[i+1] + (1<<14);
...
@ iir_custom.c:96: 		tmp_0 = ROUND_SHIFT(t0);
	ros	r3, r5
	str	r3, [fp, #-56]	@ iftmp.7_86, tmp_0
@ iir_custom.c:97: 		t1 = (int)C1 *(int)Xi + (1<<14);
	ldrsh	r3, [fp, #-36]	@ _49, C1
@ iir_custom.c:97: 		t1 = (int)C1 *(int)Xi + (1<<14);
	mul	r3, r3, r8	@ _50, _49, Xi
@ iir_custom.c:97: 		t1 = (int)C1 *(int)Xi + (1<<14);
	add	r5, r3, #16384	@ t1, _50,
@ iir_custom.c:98: 		tmp_1 = ROUND_SHIFT(t1);
	ros	r3, r5
	str	r3, [fp, #-60]	@ iftmp.8_87, tmp_1
@ iir_custom.c:99: 		t2 = (int)C2 *(int)Xi_minus_1 + (1<<14);
	ldrsh	r3, [fp, #-38]	@ _53, C2
@ iir_custom.c:99: 		t2 = (int)C2 *(int)Xi_minus_1 + (1<<14);
	mul	r3, r3, r7	@ _54, _53, Xi_minus_1
@ iir_custom.c:99: 		t2 = (int)C2 *(int)Xi_minus_1 + (1<<14);
	add	r5, r3, #16384	@ t2, _54,
@ iir_custom.c:100: 		tmp_2 = ROUND_SHIFT(t2);
	ros	r3, r5
	str	r3, [fp, #-48]	@ iftmp.9_88, tmp_2
@ iir_custom.c:101: 		t3 = (int)C3 *(int)Y[i] + (1<<14);
	ldrsh	r2, [fp, #-40]	@ _57, C3
@ iir_custom.c:101: 		t3 = (int)C3 *(int)Y[i] + (1<<14);
...
@ iir_custom.c:102: 		tmp_3 = ROUND_SHIFT(t3);
	ros	r3, r5
	str	r3, [fp, #-64]	@ iftmp.10_89, tmp_3
@ iir_custom.c:103: 		t4 = (int)C4 *(int)Yi_minus_1 + (1<<14);
	ldrsh	r3, [fp, #-42]	@ _63, C4
@ iir_custom.c:103: 		t4 = (int)C4 *(int)Yi_minus_1 + (1<<14);
	mul	r3, r3, r6	@ _64, _63, Yi_minus_1
@ iir_custom.c:103: 		t4 = (int)C4 *(int)Yi_minus_1 + (1<<14);
	add	r5, r3, #16384	@ t4, _64,
@ iir_custom.c:104: 		tmp_4 = ROUND_SHIFT(t4);
	ros	r3, r5
	str	r3, [fp, #-52]	@ iftmp.11_90, tmp_4
@ iir_custom.c:110: 		Y[i+1] = (short int) SHORT_INT_CLIPPING(tmp_0 +tmp_1 + tmp_2 + tmp_3 + tmp_4);
...
@ iir_custom.c:56: 	for(run = 0; run<10000; run++){
	...
	add	r3, r3, #1	@ tmp267, tmp268,
	str	r3, [fp, #-32]	@ tmp267, run
.L15:
@ iir_custom.c:56: 	for(run = 0; run<10000; run++){
	ldr	r3, [fp, #-32]	@ tmp269, run
	movw	r2, #9999	@ tmp270,
	cmp	r3, r2	@ tmp269, tmp270
	ble	.L38		@,
@ iir_custom.c:121: }
	nop	
	sub	sp, fp, #24	@,,

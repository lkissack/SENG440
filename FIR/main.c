...
int16_t coeffs[ MAX_FILT_LENGTH ] =
{
...
void FIR(int16_t *filter_coeffs, int16_t *input, int16_t *output, int length, int filt_length) //ints are 16bit define as such?
{
 ...
 memcpy( &insamp[filt_length - 1], input, length * sizeof(int16_t)); //copy the samples out of memory into the buffer
...
int main( void )
{
    ...
    do {
        ...
        FIR( coeffs, input, output, size, MAX_FILT_LENGTH );
